...
sub create_filelist_BOM {
    ...
    if (! -e $CE_CMD_FIND ) {
	print "ERROR: the UNIX '$CE_CMD_FIND' command was not found.\n" .
	      "       Check the value of CE_CMD_FIND \n" .
	      "       Exiting...\n\n";
	exit 1;
    }
    ...
    #--------------------------------------- 
    my @top_level_files_list = qx($CE_CMD_FIND . -maxdepth 1 -type f );
    ...
    my @dir_list = qx($CE_CMD_FIND $root_directory -maxdepth 1 -type d );
    chomp(@dir_list);
    ...
    my $cmd = "$CE_CMD_FIND $temp_dir -name \*.list";
...
sub run_file_find {
    ...
    my $cmd = "$CE_CMD_FIND './$dir_basename' -print > $dir_list_file ";
    if (system($cmd) == 0) {
